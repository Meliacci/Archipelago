...
from .rom import GBAPQuestSettings
from . import options as gbapquest_options  # rename due to a name conflict with World.options
...
class GBAPQuestWorld(World):
    """
    GBAPQuest is APQuest's Port for the GBA.
    """
    ...
    web = web_world.GBAPQuestWebWorld()

    # This is how we associate the options defined in our options.py with our world.
    # (Note: options.py has been imported as "apquest_options" at the top of this file to avoid a name conflict)
    options_dataclass = gbapquest_options.GBAPQuestOptions
    options: gbapquest_options.GBAPQuestOptions  # Common mistake: This has to be a colon (:), not an equals sign (=).
    
    settings: GBAPQuestSettings
    settings_key: ClassVar[str] = 'gbapquest_options'
    ...
    def create_item(self, name: str) -> items.GBAPQuestItem:
    ...
    def generate_output(self, output_directory: str) -> None:
        ...
        patch = rom.GBAPQUESTProcedurePatch(player=self.player, player_name=self.multiworld.player_name[self.player])
    ...
    def fill_slot_data(self) -> Mapping[str, Any]:
        ...
        return self.options.as_dict(
            "hard_mode", "hammer", "extra_starting_chest",
             # "confetti_explosiveness", "player_sprite"
        )
